...
'$builder_hook'(profilercc:lib('lib/profilercc')).
'$builder_hook'(profilercc:prepare_build_bin) :-
    gen_decl_auto.
'$builder_hook'(profilercc:clean_bin) :- !,
    % TODO: Remove hook if lib/2 is treated by clean_bin
    clean_tree(~bundle_path(ciaodbg, 'lib/profilercc')).
...
gen_decl_auto :-
    ...
    string_to_file(T, ~bundle_path(ciaodbg, 'lib/profilercc/profilercc_decl_auto.pl')).
