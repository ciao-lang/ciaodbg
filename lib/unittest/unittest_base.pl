...
test_redirect_chn(show, _Std, _, none).
...
test_redirect_chn(null, _, _, file(File)) :-
    mktemp_in_tmp('tmpXXXXXX', File). % TODO: add null redirection to io_port_reify?
test_redirect_chn(stdout, _, _, stdout).
...
test_redirect_contents(show, _Std, _, dumped).
...
test_redirect_contents(null, _Std, _, ignored).
test_redirect_contents(stdout, _Std, _, redirected_to_stdout).
